...
module.exports = class RunMonitorSet {
  ...
  logMonitorSetEnd ({ monitorSetConfig, elapsedTime, success, results }) {
    ...
    if (success) return `  ${colors.green('✔')} ${time}`
    return `  ${colors.red('✕')} ${time}`
  }

  logMonitorSetResult ({ monitorSetConfig, elapsedTime, success, results }) {
    const slow = toMS(monitorSetConfig.slowThreshold, '30s')
    const time = colors[elapsedTime > slow ? 'yellow' : 'green'](`(${logElapsedTime(elapsedTime)})`)
    if (success) return `${colors.green('✔')} ${colors.underline(monitorSetConfig.id)} ${time}:`
    return `${colors.red('✕')} ${colors.underline(monitorSetConfig.id)} ${time}:`
  }
